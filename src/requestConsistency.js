...
const extractDataset = async (iexecIn, iexecDatasetFilename) => {
  if (
    typeof iexecDatasetFilename === "string" &&
    iexecDatasetFilename.length > 0
  ) {
...
const extractApiKey = (paramSet, dataset) => {
  ...
  if (dataset !== undefined) {
    if (paramSet.dataset.toLowerCase() !== dataset.address.toLowerCase())
      throw Error(
        "The dataset used does not match dataset specified in the paramset"
      );
    if (callId !== dataset.callId) {
      throw Error(
        "Computed callId does not match dataset's callId \n" +
